...
func main() {
	...
	r.POST("/translation", handlePostRequest)
	r.POST("/aliyunoss", uploadedPostRequest)
...
func handlePostRequest(c *gin.Context) {
	...
	default:
		// 不删除数据，直接返回或做其他处理
		fmt.Println("No directory will be deleted.")
		delStatus = "未删除商品下载目录"
	}
...
func uploadedPostRequest(c *gin.Context) {
	...
	var urllist []string
	for i, imagename := range TranslateList {
		log.Print(i)
		ts := Timestamps() // 假设这个函数返回字符串格式的时间戳
		objectKey := "translatedir/" + requestData.ID + "/" + ts + "/" + imagename
		log.Print("objectKey:", objectKey)
		// localFilePath := currentPath + "/" + outputDir + "/" + imagename
		localFilePath := outputDir + "/" + imagename
	...
	// 返回成功响应或其他你需要的响应数据
	c.JSON(200, gin.H{"status": "success", "ossurl": urllist, "currentPath": currentPath, "outputDir": outputDir, "TL_Name": TranslateList, "delstatus": delStatus})
}
...
/*
	func test_translate() {
		...
		// 输出目录
		downloadDir := "downloadDir" + "/" + "09343436"
		outputDir := "outputDir" + "/" + "09343436"
		SL := languagecodes.ENGLISH
		TranslateList, err := TranslateImages(imageURLs, downloadDir, outputDir, languagecodes.DETECT_LANGUAGE, SL)
		if err != nil {
			log.Fatal(err)
		}
		log.Print(TranslateList)
